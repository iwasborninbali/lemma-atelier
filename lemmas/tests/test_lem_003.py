...
M-инвариантных максимальных S (n = 5, 6, 7; инверсия, отражение, поворот порядка 4, поворотное отражение порядка 6 — v2) и непустоту; (2) ТОЧНОСТЬ границ — множества,
...
class TestLem003(unittest.TestCase):
    def test_1_random_cases_nonvacuous(self):
        ...
        for _ in range(45):
            ...
            if not (L.kind(M) in ('rotation4', 'rotoreflection6') and n % 2 == 0):   # при чётном n ось/центр вне клеток: S = ∅ законно
                self.assertGreater(len(S), 0, (L.kind(M), n))
        self.assertEqual(set(seen), {"inversion", "reflection", "rotation4", "rotoreflection6"}, "тест пуст по одному из видов (v2: + S6)")
